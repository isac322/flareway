...
func TestTranslateMixedHostnameNestedPublicCarveOut(t *testing.T) {
	...
	tools := namedRouteRule("api-tools", "/backend-api/tools", "api")
	in.HTTPRoutes = []gatewayv1.HTTPRoute{{
		...
		Spec: gatewayv1.HTTPRouteSpec{
			...
			Rules:           []gatewayv1.HTTPRouteRule{dashboard, api, tools},
		},
	}}
	app := accessApplication("dashboard", "HTTPRoute", "tools", "dashboard")
	in.AccessApplications = []v1alpha1.AccessApplication{app}
	...
	if len(compiled.Bypass) != 2 || compiled.Bypass[0].Path != "/backend-api/tools" || compiled.Bypass[1].Path != "/v1" {
...
func TestPathScopeAccessApplicationShadowsParentRouteWithoutConsumingIt(t *testing.T) {
	...
	in.HTTPRoutes = []gatewayv1.HTTPRoute{{
		...
		Spec: gatewayv1.HTTPRouteSpec{
			CommonRouteSpec: gatewayv1.CommonRouteSpec{ParentRefs: []gatewayv1.ParentReference{{Name: "gateway"}}},
			Rules:           []gatewayv1.HTTPRouteRule{namedRouteRule("reports", "/", "backend")},
		},
	}}
	parent := accessApplication("parent", "Gateway", "gateway", "http")
	parent.CreationTimestamp = metav1.NewTime(time.Unix(1, 0))
	overlay := accessApplication("overlay", "HTTPRoute", "routes", "reports")
	...
	if len(parentDomain.VirtualHosts[0].Routes) != 1 || parentDomain.VirtualHosts[0].Routes[0].Source.RuleName != "reports" ||
		len(overlayDomain.VirtualHosts[0].Routes) != 1 || overlayDomain.VirtualHosts[0].Routes[0].Source.RuleName != "reports" {
...
func TestTranslateMixedHostnameWithoutGrantDropsPublicRules(t *testing.T) {
	...
	condition := findRouteCondition(statuses.HTTPRoutes[types.NamespacedName{Namespace: "default", Name: "tools"}], gatewayv1.RouteConditionPartiallyInvalid)
