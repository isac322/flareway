	...
	gateway := &ir.Gateway{
		...
		Domains: []ir.ProtectionDomain{
			...
			},
			{
				...
				Access:       &ir.AccessGuard{AUDs: []string{"aud-codex"}, TeamName: "team"},
...
func TestCompileCanonicalizesMultipleAUDsForHost(t *testing.T) {
	gateway := &ir.Gateway{
		...
		Listeners:  []ir.Listener{{Name: "http", Exposure: ir.ExposurePublic}},
		Domains: []ir.ProtectionDomain{{
			Name: "redash", ListenerName: "http", EnvoyPort: 18080, Protected: true, Guard: ir.GuardForwarding,
			Access: &ir.AccessGuard{
				AUDs:     []string{"redash-reports", "", "redash-admin", "redash-reports"},
				TeamName: "team",
			},
			VirtualHosts: []ir.VirtualHost{{Hostname: "redash.example.com"}},
		}},
	}
	...
	if len(config.Ingress) != 2 || config.Ingress[0].OriginRequest == nil || config.Ingress[0].OriginRequest.Access == nil {
		t.Fatalf("Redash ingress = %#v", config.Ingress)
	}
	if got, want := config.Ingress[0].OriginRequest.Access.AUDTag, []string{"redash-admin", "redash-reports"}; !slices.Equal(got, want) {
	...
	gateway.Domains[0].Access.AUDs = []string{"redash-admin", "redash-reports", "redash-admin"}
