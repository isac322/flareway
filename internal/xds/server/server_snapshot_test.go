...
func TestSnapshotRequiredTypesUsesActualSDSReference(t *testing.T) {
	...
	if _, ok := required[string(resourcev3.SecretType)]; !ok { t.Fatal("actual SDS-referenced Secret type was not required") }
	...
	if err != nil { t.Fatal(err) }
	required = snapshotRequiredTypes(snapshot)
	if _, ok := required[string(resourcev3.SecretType)]; ok { t.Fatal("unreferenced Secret type incorrectly became required") }
}
