...
func TestDevicePostureIntegrationRemoteErrorRecordsStatus(t *testing.T) {
	...
	reconciler := &DevicePostureIntegrationReconciler{Client: kube, Scheme: testScheme(), NewCloudflareClient: func(string, string) (flarecloudflare.AccessAPI, error) { return &failingPostureAPI{integrationErr: remoteErr}, nil }, Now: func() time.Time { return time.Unix(10, 0) }}
	...
	if err := kube.Get(ctx, client.ObjectKeyFromObject(object), current); err != nil { t.Fatal(err) }
	if current.Status.ObservedGeneration != current.Generation { t.Fatalf("observedGeneration = %d, want %d", current.Status.ObservedGeneration, current.Generation) }
	ready := statusutil.FindCondition(current.Status.Conditions, "Ready")
	if ready == nil || ready.Status != metav1.ConditionFalse || ready.Reason != "CloudflareError" { t.Fatalf("Ready condition = %#v, want False/CloudflareError", ready) }
}
...
func TestDevicePostureRuleRemoteErrorRecordsStatus(t *testing.T) {
	...
	reconciler := &DevicePostureRuleReconciler{Client: kube, Scheme: testScheme(), NewCloudflareClient: func(string, string) (flarecloudflare.AccessAPI, error) { return &failingPostureAPI{ruleErr: remoteErr}, nil }, Now: func() time.Time { return time.Unix(10, 0) }}

	_, err := reconciler.Reconcile(ctx, ctrl.Request{NamespacedName: client.ObjectKeyFromObject(object)})
	if !errors.Is(err, remoteErr) { t.Fatalf("Reconcile() error = %v, want %v", err, remoteErr) }
	...
	for _, name := range []string{"Accepted", "Ready"} {
		...
		if condition == nil || condition.Status != metav1.ConditionFalse || condition.Reason != "CloudflareError" { t.Fatalf("%s condition = %#v, want False/CloudflareError", name, condition) }
	}
}
...
func (f *failingPostureAPI) CreateDevicePostureIntegration(context.Context, flarecloudflare.DevicePostureIntegrationInput) (flarecloudflare.DevicePostureIntegration, error) { return flarecloudflare.DevicePostureIntegration{}, f.integrationErr }
func (f *failingPostureAPI) CreateDevicePostureRule(context.Context, flarecloudflare.DevicePostureRuleInput) (flarecloudflare.DevicePostureRule, error) { return flarecloudflare.DevicePostureRule{}, f.ruleErr }

func testScheme() *runtime.Scheme {
	scheme := runtime.NewScheme()
	if err := v1alpha1.AddToScheme(scheme); err != nil { panic(err) }
	if err := corev1.AddToScheme(scheme); err != nil { panic(err) }
	return scheme
}
