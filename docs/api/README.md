...
Individual resource examples and a complete neutral workload mapping live in [`config/samples/`](../../config/samples/). See [`workload_application.yaml`](../../config/samples/workload_application.yaml) for an end-to-end example.

All IDs, tokens, hostnames, and account details in the sample files are placeholders.
