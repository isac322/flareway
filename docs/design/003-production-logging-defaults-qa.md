...
5. **CI 분기**: e2e 배포 파이프라인은 렌더된 Kustomize 인자를 sed로 `--zap-log-level=debug`로 바꿔 V(1) Cloudflare 요청 로그를 트riage용으로 보존하고, conformance는 출하 기본값 그대로 실행해 실클러스터에서 프로덕션 로깅을 검증한다 `[D]`. e2e 아티팩트 redaction은 베이스(c7f2b9e)의 key 정규식과 토큰 그룹을 둔 Bearer 정규식을 모두 **원문**에 매칭하고 두 패스의 값/토큰 구간 합집합을 마스킹한다 — 구성상 두 베이스 패스 중 하나라도 가리던 문자는 모두 가려지며, key 뒤 Bearer 토큰 누출을 추가로 막는다. JSON 유효성 보존·이스케이프 중첩 값 redact는 이 PR의 목표가 아니다(선존 한계, §9) `[D]→[C]`(§5 R7).
...
1. **e2e redaction 정규식이 JSON 로그에서 시크릿을 누출한다** `[E]`: 현재 `.github/workflows/e2e.yaml`의 정규식을 그대로 실행하면 `{"authorization":"Bearer tok"}`는 토큰이 남고(key 패스가 `"Bearer`만 소비한 뒤 Bearer 패스가 더는 매칭되지 않음), `"api_token":"x y"`/`"x,y"`는 공백·콤마 뒤가 노출된다. 콘솔 포맷에서도 같은 누출이 있었으나, 전환 후 스트림이 100% JSON이 되어 노출이 극대화된다 — 이 PR은 key 뒤 Bearer 토큰 누출을 고친다(R7); 공백·콤마 뒤 꼬리 누출은 베이스와 동일한 선존 한계로 남는다(§9) `[C]`.
...
- **R7** e2e.yaml 배포 파이프라인은 렌더된 Kustomize의 manager args에서 `--zap-log-level=info`를 `=debug`로 sed 치환(V(1) 트riage 스트림 보존)하고 치환 성공을 grep으로 단언해 조용한 실패를 막는다. conformance(`hack/run-conformance.sh`)는 출하 기본값 유지. e2e redaction 스크립트: 베이스(c7f2b9e)의 key 정규식(변경 없음)과 토큰을 캡처 그룹으로 둔 Bearer 정규식(`\bBearer\s+([A-Za-z0-9._~+/=-]+)`)을 둘 다 **원문**에 매칭하고, key 값 구간은 `"[REDACTED]"`, Bearer 토큰 구간은 `[REDACTED]`로 마스킹하되 겹치는 구간은 병합한다(겹치면 `"[REDACTED]"`). 두 패스가 서로의 입력을 바꾸지 않으므로 구성상 베이스의 어느 패스가 가리던 문자도 모두 가려진다(`git diff c7f2b9e -- .github/workflows/e2e.yaml`).
  - **R7 경과 1 `[C]`(QA-21)**: 첫 재작성은 이스케이프 따옴표 구분자(`(?:\\?")?`), 따옴표 스타일 보존 마스크, 따옴표 없는 값의 lookahead 대안으로 key 패스를 바꿔 이스케이프 중첩 JSON redact와 JSON 유효성 보존을 노렸다. 최종 보안 리뷰와 차등 퍼즈가 매 단계 베이스 대비 커버리지 회귀를 찾았다 — 선행 백슬래시 값(`client_secret=\abc`, `tunnel_token=\"abc`)과 베이스의 `"[^"]*"`가 `\"`를 넘어 소비하던 값을 놓쳤다. 그래서 key 정규식은 베이스로 되돌렸고 "이스케이프 중첩 값 redact"와 "redaction 후 JSON 유효" 목표는 철회했다 — 둘 다 베이스와 동일한 선존 한계다(§9).
  - **R7 경과 2 `[C]`(최종 보안 리뷰)**: 중간 단계인 "베이스 key 패스 + Bearer 패스 선실행" 순차 치환은 겹침 반례를 남겼다: `Bearer api_token="secret"`, `Bearer client_secret: "secret"`에서 Bearer 토큰 클래스가 key 이름을 먼저 소비해 key 패스가 값을 보지 못하지만 베이스는 값을 가린다. 두 패스를 원문 매칭 + 구간 합집합으로 바꿔 순서 의존을 없앴다. 차등 퍼즈 40,000 라인(이후 70,000 라인으로 확장, R7 경과 3)에서 베이스 대비·Bearer-선실행 대비 새 출력에만 남는 토큰 0건, 베이스 대비 새로 깨진 JSON 0건(QA-21).
  - **R7 경과 3 `[C]`(최종 보안 리뷰)**: 원문 `bearer.finditer`는 겹치지 않는 매칭이라 `client_secret=Bearer Bearer secret`에서 첫 `Bearer Bearer`만 잡고 뒤 `secret`을 남겼다(베이스는 두 번째 패스에서 가림). Bearer 정규식을 `\bBearer(?=\s+(토큰))` lookahead로 바꿔 모든 `Bearer` 시작점에서 토큰을 잡는다. key 치환은 따옴표·대괄호만 삽입하므로 새 `Bearer`나 토큰 문자를 만들지 않고, 따라서 베이스 두 번째 패스가 가리는 바이트는 전부 원문의 어떤 `Bearer` 시작점 토큰 안에 있다(보안 리뷰 동의). 차등 퍼즈를 반복 `Bearer`·`client_secret=Bearer` 접두 30,000 라인을 더해 70,000 라인으로 확장: 베이스 대비 잔존 토큰 0, JSON 유효성 악화 0 `[E]`.
- **R8** 문서: 차트 README(값 테이블 행 + Logging 섹션: 매핑 표, 샘플러, klog/grpc 텍스트 예외, 복원 레시피, 세 키만 노출, raw `--zap-log-level≥8`의 Secret 본문 로깅 경고 — 본문은 레벨 8에서 1024바이트, 9에서 10240바이트로 잘리고 10부터 전체 기록(client-go `rest/request.go`)), docs/operations/upgrade.md(마이그레이션: console→JSON, debug→info, 연속행 스택트레이스→`"stacktrace"` 필드, 샘플러, klog 텍스트 예외(포맷 불변), Helm·Kustomize 복원 레시피, 로컬 `go run ./cmd --zap-devel=true`), docs/operations/troubleshooting.md(debug 활성화: Helm `--set`, Kustomize JSON6902 append 또는 전체 args 편집 — strategic-merge args 치환 금지, 라이브 deploy에 `kubectl patch`, `logging.level=1`로 요청당 라인, raw 플래그 ≥8 경고, klog/grpc 텍스트 예외와 `jq fromjson?` 필터링), bug_report.yml 힌트. `warn`은 절대 문서화하지 않는다(플래그가 거부).
...
| QA-21 | R7: 수정된 redaction 스크립트가 베이스(c7f2b9e) redaction보다 누출하지 않고 key 뒤 Bearer 누출을 막음 | 베이스 블록, 중간 Bearer-선실행 블록, PR 블록에 같은 입력 투입(차등 퍼즈 40,000 라인(이후 70,000 라인으로 확장, R7 경과 3)): 민감 키 변형 11종 × 구분자 8종 × 영숫자·백슬래시·따옴표·이스케이프 따옴표·공백·콤마·중괄호·Bearer 토큰을 섞은 값, 평문·JSON 문자열 내장·JSON 키 형태 20,000 라인 + Bearer/`authorization` 접두와 겹침 픽스처 20,000 라인; 고정 사례 `{"authorization":"Bearer abc.def.ghi"}`, `Authorization: Bearer abcdef`, `Bearer api_token="secret"`, `Bearer client_secret: "secret"`, `client_secret=\abc`, `tunnel_token=\"abc` | 베이스 대비 및 Bearer-선실행 대비 새 출력에만 남는 3자 이상 값 토큰 0건; key 뒤 Bearer 토큰이 redact됨; 베이스가 유효하게 둔 JSON을 깨뜨리지 않음 | 없음 — 아티팩트 안전 주장을 차단 | throwaway(스크립트 수정은 PR에 포함) | FAIL → 번복 후 PASS `[C]` — 첫 재작성은 선행 백슬래시 값 등에서, Bearer-선실행은 `Bearer api_token="secret"` 겹침에서 베이스 대비 회귀(§5 R7 경과 1·2); 최종(원문 매칭 + 구간 합집합): 베이스 대비 0건, Bearer-선실행 대비 0건, 베이스 대비 JSON 유효성 회귀 0건, 베이스 누출 해소(`{"authorization":"Bearer abc.def.ghi"}`, `Authorization: Bearer abcdef`); 잔여 누출(이스케이프 중첩 값, `"key": "v"`, 공백·콤마 뒤 꼬리 등)은 베이스와 동일한 선존 공백(§9) → §7.1 (b) |
...
- **(b) e2e redaction 누출 `[E]`→`[C]`**: 베이스 스크립트는 key 패스를 Bearer 패스보다 먼저 실행해, key 패스가 `"Bearer`만 소비한 뒤 남은 토큰을 Bearer 패스가 더는 잡지 못했다(`{"authorization":"Bearer tok"}` → `tok` 누출). 첫 수정(이스케이프 따옴표 구분자 + 따옴표 스타일 보존 마스크 + 따옴표 없는 값 lookahead)은 베이스가 잡던 선행 백슬래시 값(`client_secret=\abc`, `tunnel_token=\"abc`)과 `\"`를 넘는 값을 놓쳤고, 이어진 Bearer-선실행 순차 치환은 `Bearer api_token="secret"`처럼 Bearer 토큰이 key 이름을 삼키는 겹침에서 값을 놓쳤다(최종 보안 리뷰 + 차등 퍼즈). 최종 수정: 베이스 key 정규식과 토큰 그룹 Bearer 정규식을 원문에 매칭하고 구간 합집합을 마스킹(§5 R7). 재실행: 차등 퍼즈 40,000 라인(이후 70,000 라인으로 확장, R7 경과 3)에서 베이스·Bearer-선실행 대비 커버리지 회귀 0건, 베이스 대비 JSON 유효성 회귀 0건, key 뒤 Bearer 누출 해소(QA-21). 이스케이프 중첩 값 누출과 redaction 후 JSON 파손은 베이스와 동일한 선존 한계로 남는다(§9).
- **(c) QA 기준 정정 `[C]`**:
...
- **범위 외(후속 과제)** `[E]`: redaction의 선존 공백 — 베이스(c7f2b9e) regex와 최종 regex가 동일하게 놓치므로(차등 퍼즈 QA-21) 이 변경의 회귀가 아니며 여기서 고치지 않는다. (1) `resources.yaml`의 `audTags` 키(CloudflareTunnel Access AUD 목록): YAML 블록(`audTags:` 다음 줄 `- …`)·플로우(`audTags: ["…"]`)·JSON(`"audTags":[…]`) 모두 누출 — 키 대안 `aud` 바로 뒤에 구분자가 와야 해서 매칭되지 않는다. (2) YAML 블록 값: `key:` 뒤 줄바꿈 리스트는 `\s*`가 줄을 넘어 `-`만 마스킹하고, `|` 리터럴 블록은 `|`만 마스킹한다(무해한 `authorization:` 블록 YAML도 훼손). (3) JSON 배열·객체 값(`"aud":[…]`, `"tunnel_token":{…}`): 누출 + JSON 파손. (4) 이스케이프 중첩 JSON(`"error":"{\"api_token\":\"abc\"}"`, 이중 이스케이프 `\\\"api_token\\\"` 포함): 키 뒤 `\`가 구분자로 인식되지 않아 누출. (5) 콜론 뒤 공백이 있는 `"key": "v"`: key 패스가 `"`·`:`만 소비해 값이 누출. (6) JSON 문자열 값의 공백·콤마 뒤 꼬리(`"api_token":"x y"` → `y`): key 패스가 닫는 따옴표를 구분자로 쓰고 `[^\s,}]+`로 첫 조각만 마스킹. (7) JSON 유효성: 치환이 `"key""[REDACTED]"` 형태를 만들어 redact된 JSON 라인은 파싱되지 않는다. 문서는 e2e 아티팩트에 AUD가 포함되면 안 된다고 명시하므로 별도 후속 과제로 추적한다(구분자 `[ \t]*` + YAML 시퀀스/블록 처리, 또는 `resources.yaml` 구조적 redaction).
...
- **redaction 커버리지 회귀(보안) — 재설계로 해소**: 최종 보안 리뷰가 첫 재작성 regex(이스케이프 따옴표 구분자·따옴표 스타일 보존 마스크·따옴표 없는 값 lookahead)에서 베이스가 잡던 선행 백슬래시 값(`client_secret=\abc`, `tunnel_token=\"abc`)을 놓치는 회귀를, 이어 베이스 key 패스 + Bearer-선실행 단계에서 겹침 반례(`Bearer api_token="secret"`, `Bearer client_secret: "secret"`)를 찾았다. 베이스 key 정규식과 토큰 그룹 Bearer 정규식을 원문에 매칭해 구간 합집합을 마스킹하는 방식으로 해소했다 — 차등 퍼즈 40,000 라인(이후 70,000 라인으로 확장, R7 경과 3)에서 베이스·Bearer-선실행 대비 회귀 0건, JSON 유효성 회귀 0건(§5 R7, QA-21).
- **최종 동작 리뷰 — 회귀 없음**: 베이스/head 복원 출력 parity, klog 텍스트 라인 동일, Helm·Kustomize 렌더는 세 `--zap-*` 인자만 다르다.
