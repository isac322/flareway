...
metadata: {name: runbear}
spec:
  accountId: "<32hex>"
  credentials:
    apiTokenSecretRef: {name: cf-api-token, namespace: flareway-system, key: token}
  grants:                       # 테넌트 경계. 매칭되는 항목이 없으면 그 namespace의 참조는 거부
    - namespaceSelector: {matchLabels: {flareway.bhyoo.com/tenant: llm-proxy}}
      hostnames: ["*.runbear.io"]          # listener hostname 허용 패턴
      zones: ["runbear.io"]
      ...
      backends:                             # ReferenceGrant에 더해 적용하는 SSRF 경계
        ...
        # selector: {matchLabels: {flareway.bhyoo.com/backend-for: llm-proxy}}
        kinds: [Service]
...
metadata: {name: cc-lb, namespace: llm-proxy}
spec:
...
```yaml
kind: AccessPolicy
metadata: {name: allow-developers-warp, namespace: flareway-system}
spec:
  accountRef: {name: runbear}
  ...
  require:
    - devicePosture: {ruleRef: {name: warp-connected}} # DevicePostureRule CR 또는 {externalId}
  exclude: []
  sessionDuration: 720h
...
spec:
  ...
  profile:
    ...
    # Custom일 때만:
    # match: 'identity.email matches ".*@runbear.io"'
    # precedence: 100
...
- cc-lb 첫 이전: Terraform profile을 `ObserveOnly`로 두고 `status`에 "would-apply diff"만 보고(기획서 §9.3).
...
AUD가 바뀌거나 보호 경계를 보존할 수 없으면 block-first로 처리한다: 기존 영역 403 → 새 app 생성 및 AUD 검증 → ingress/Envoy 적용 → allow/deny probe → 서비스 재개. v1에는 migration mode가 없으며 한 rule에 이전 AUD와 새 AUD를 동시에 허용하지 않는다.
...
## 11. cc-lb 예제 (기획서 §13 목표 토폴로지)

실제 적용 가능한 전체 예제는 [`config/samples/workload_cc_lb.yaml`](../../config/samples/workload_cc_lb.yaml)에 있다. 생성 CRD와 Gateway API Go type으로 strict decode한다. 토큰, account ID, remote ID, hostname은 예시 값이다.

| 기존 역할 | Flareway 표현 |
|---|---|
| admin hostname 전체 Access | `Gateway` listener `admin` + `AccessApplication.targetRefs[]`의 Gateway/sectionName |
| proxy hostname `/v1`만 공개 | 별도 listener/HTTPRoute + `unprotectedHostnames` grant |
| Terraform 소유 Google IdP/allow/deny 정책 | `IdentityProvider`와 `AccessPolicy`를 `ObserveOnly`, `externalRef`, `deletionPolicy: Orphan`으로 참조 |
...
codex-lb와 cliproxyapi의 P⊇Q 혼합 모델은 각각 [`workload_codex_lb.yaml`](../../config/samples/workload_codex_lb.yaml), [`workload_cliproxyapi.yaml`](../../config/samples/workload_cliproxyapi.yaml)에 있다. 두 예제 모두 root dashboard를 보호하고 API prefix를 public carve-out으로 선언하며, child bypass application은 사용자가 직접 만들지 않고 Flareway가 관리한다.

---

## 12. 검증 모델 `[U-4]`

공식 conformance는 제품 검증의 정점이 아니라 **Gateway API 규격 계층**이다. Cloudflare 기능을 제거한 conformance mode가 Tunnel, DNS, Access, WARP를 증명할 수 없기 때문이다. 제품 검증의 정점은 소수의 실제 Edge e2e 계약이다. 공식 fixture의 `*.example.com`을 테스트 zone의 Edge로 보낼 수 없고 프로덕션 경로에 test-only host 우회를 넣지 않으므로 conformance-through-edge는 제외한다.
...
| ID | 상태 |
|---|---|
| D-01 `/v1` vs all-path | **확정**: cc-lb proxy는 `/v1`만 공개 |
...
| D-04 API group/저장소 | **확정**: `flareway.bhyoo.com`, 공개 `github.com/isac322/flareway` |
| D-05 adoption/credential | 운영 이전은 별도 승인. 구현은 ID+expected attribute 검증과 Secret 참조 제공 |
