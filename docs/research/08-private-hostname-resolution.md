...
When exposing private hostnames (e.g. `admin.cc-lb.internal`) to WARP clients via Cloudflare Tunnel, **`cloudflared` does not perform application-level DNS resolution at connection time**. Instead, Cloudflare Zero Trust splits private hostname routing into two distinct phases:

1. **Synthetic Resolution at Gateway Edge:** When a WARP client queries DNS for `admin.cc-lb.internal`, Cloudflare Gateway assigns a temporary "initial resolved IP" (drawn by default from `172.64.128.0/20`). Concurrently, Gateway queries `cloudflared` over the tunnel using a dedicated virtual DNS origin (`2606:4700:0cf1:2000::1:53`) to obtain the "real" origin IP.
...
- Incoming private network TCP streams are dispatched by QUIC/HTTP2 connection managers (`connection/quic_connection.go:252` and `connection/http2.go:145`) into `Proxy.ProxyTCP`:
  ...
  ```
- **The edge sends an IP:port, never a hostname:** `req.Dest` is strictly parsed via `netip.ParseAddrPort(req.Dest)`. If the edge sent `admin.cc-lb.internal:80`, `ParseAddrPort` would immediately fail with an error.
...
.:53 {
    hosts {
        127.0.0.1 admin.cc-lb.internal
        127.0.0.1 *.cc-lb.internal
        fallthrough
    }
...
3. **Wildcard support:** Excellent. CoreDNS `hosts` plugin natively supports wildcards (`*.cc-lb.internal`). CoreDNS `template` plugin can handle arbitrary pattern matching.
...
### Description
Modify the cluster-wide CoreDNS ConfigMap (`kube-system/coredns`) by adding `hosts` or `rewrite` rules pointing `admin.cc-lb.internal` to the Envoy Service ClusterIP.
...
2. Create a Cloudflare Gateway DNS policy (`POST /accounts/{account_id}/gateway/rules`):
   - `filters: ["dns"]`
   - `action: "override"`
   - `traffic: "dns.fqdn == \"admin.cc-lb.internal\""`
   - `rule_settings`:
     ```json
     {
...
### Evaluation
1. **Does it work at all?** **YES.** When the client queries `admin.cc-lb.internal`, Gateway directly returns `10.96.123.45`. The client opens a TCP connection to `10.96.123.45:443`. WARP routes the flow over the tunnel. `cloudflared` receives `Dest: "10.96.123.45:443"` and dials the ClusterIP directly. **No in-cluster DNS resolution occurs.**
2. **Loopback reachability:** Dials the ClusterIP via kube-proxy / CNI, not `127.0.0.1`.
3. **Wildcard support:** Gateway DNS rule traffic expressions support wildcards: `dns.fqdn.matches(".*\\.cc-lb\\.internal")`.
4. **Platform prerequisites:**
...
- **Evidence:**
  ...
  - CoreDNS sidecar supports wildcards (`*.cc-lb.internal`) and dynamic reloading via ConfigMap without pod restarts.
