...
When Flareway exposes a private hostname (e.g. `app.example.com`) via an Access Application backed by a public DNS CNAME pointing to `<tunnel-id>.cfargotunnel.com`:
- **In Exclude Mode (Default WARP profile)**:
  1. The client resolves `app.example.com` via standard DNS / Gateway DoH.
  ...
  5. **Result**: In Exclude mode, `app.example.com` **does not** need to be in the Split Tunnel exclude or include list.
- **In Include Mode (Strict Zero Trust profile)**:
  ...
  2. If `app.example.com` is *not* in the include list, the client routes the TCP handshake direct to the public internet. While Cloudflare Edge still answers, traffic bypasses Gateway inspection and device posture metadata carried inside the WARP tunnel may be lost.
  3. **Result**: In Include mode, either:
     - The explicit domain `app.example.com` (or wildcard `*.example.com`) must be added as `{ host: "app.example.com" }` to the profile's `include` list, OR
